...
struct ggml_tensor * forward_batch_wo_cache_flash_attn_train_grad_checkpointing(
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        clr_buf(0);
        use_buf(prs); struct ggml_tensor * t02 = expand(gf, ggml_rms_norm     (ctx0, cur));                                    assert_shape_2d(t02, n_embd, N*n_batch);
        ...
        use_buf(prs); struct ggml_tensor * t22 = expand(gf, ggml_rms_norm     (ctx0, t21));                                    assert_shape_2d(t22, n_embd, N*n_batch);
    ...
    for (int k = 0; k < n_layer; ++k) {
        ...
        if (il < avail_begin) {
            ...
            for (int i = begin; i < end; ++i) {
                ...
                clr_buf(0);
                use_buf(prs); struct ggml_tensor * t02 = expand(gb, ggml_rms_norm     (ctx0, cur));                                    assert_shape_2d(t02, n_embd, N*n_batch);
                ...
                use_buf(prs); struct ggml_tensor * t22 = expand(gb, ggml_rms_norm     (ctx0, t21));                                    assert_shape_2d(t22, n_embd, N*n_batch);
